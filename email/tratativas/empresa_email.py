...
# Função para encontrar o email de uma empresa
def encontrar_email_empresa(razao_social):
    query = quote_plus(razao_social) + "+email"
    ...
    if requisicao.status_code == 200:
        email = BeautifulSoup(requisicao.text, "html.parser")  # criar um objeto BeautifulSoup para fazer o parsing do conteúdo HTML

        email_empresa = email.find("cite", class_="apx8Vc tjvcx GvPZzd cHaqb")  # encontrar o elemento <cite> na página

        if email_empresa:
            return email_empresa.get_text()
        else:
...
# Iterar pelas empresas e encontrar o email de cada uma
for empresa in dados_empresas:
    razao_social = empresa["razao_social"]
    email = encontrar_email_empresa(razao_social)
    resultado_empresa = {
